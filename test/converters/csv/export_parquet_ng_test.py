...
@mock_s3
def test_ParquetExporter(event_ng, husholdninger_single):
    (prefix, file) = husholdninger_single()
    event = event_ng(prefix, chunksize=2)
    exporter = ParquetExporterNg(event)
    exporter.export()
    expected = pd.read_csv(file)
    output_prefix = event["payload"]["output_dataset"]["s3_prefix"].replace(
        "%stage%", "intermediate"
    )
...
@mock_s3
def test_ParquetExporter_no_chunks(event_ng, husholdninger_single):
    (prefix, file) = husholdninger_single()
    event = event_ng(prefix, chunksize=None)
    exporter = ParquetExporterNg(event)
    exporter.export()
    expected = pd.read_csv(file)
    output_prefix = event["payload"]["output_dataset"]["s3_prefix"].replace(
    ...
    result = pd.read_parquet(
        f"s3://{BUCKET}/" + output_prefix + event["task"] + "/husholdninger.parquet.gz"
    )
    # result is a subset of expected
    assert len(result.merge(expected)) == len(result)
...
@mock_s3
def test_ParquetExporter_with_schema(event_ng, schema_ng):
    (prefix, file) = schema_ng()
    event = event_ng(prefix, chunksize=None, schema=schema)
    exporter = ParquetExporterNg(event)
    exporter.export()
    ...
    result = pd.read_parquet(
        f"s3://{BUCKET}/" + output_prefix + event["task"] + "/schema_ng.parquet.gz"
    )
...
@mock_s3
def test_ParquetExporter_with_schema_wrong_number(event_ng, schema_wrong_ng):
    (prefix, file) = schema_wrong_ng()
    event = event_ng(prefix, chunksize=None, schema=schema)
    exporter = ParquetExporterNg(event)
    with pytest.raises(
        ConversionError,
...
def export_and_read_result(event, outputprefix):
    exporter = ParquetExporterNg(event)
    exporter.export()
    output_prefix = event["payload"]["output_dataset"]["s3_prefix"].replace(
        "%stage%", "intermediate"
    ...
    result_path = f"{BUCKET}/{output_prefix}{event['task']}/{outputprefix}*"
...
@mock_s3
def test_ParquetExporter_valid_dates_not_chunked(event_ng, dates_file):
    (prefix, file) = dates_file()
    event = event_ng(prefix, chunksize=None, schema=schema_dates)
    result = export_and_read_result(event, "schema_dates")
...
@mock_s3
def test_ParquetExporter_valid_dates_chunked(event_ng, dates_file):
    (prefix, file) = dates_file()
    event = event_ng(prefix, chunksize=2, schema=schema_dates)
    result = export_and_read_result(event, "schema_dates")
    assert result["year_column"][0].year == 1678
    assert result["year_column"][3].year == 2262
    assert result["date_column"][0].year == 1678
...
def test_ParquetExporter_invalid_year_too_early_not_chunked(
    event_ng, dates_file_year_too_early
):
...
def test_ParquetExporter_invalid_year_too_early_chunked(
    ...
    (prefix, file) = dates_file_year_too_early()
    event = event_ng(prefix, chunksize=1, schema=schema_dates)
    exporter = ParquetExporterNg(event)
    result = exporter.export()
    assert result["status"] == "CONVERSION_FAILED"
    assert len(result["errors"]) == 1
...
def test_ParquetExporter_invalid_year_too_late_not_chunked(
    event_ng, dates_file_year_too_late
):
...
@mock_s3
def test_ParquetExporter_invalid_year_too_late_chunked(
    event_ng, dates_file_year_too_late
):
    (prefix, file) = dates_file_year_too_late()
    event = event_ng(prefix, chunksize=1, schema=schema_dates)
...
def test_ParquetExporter_date_with_string_not_chunked(
    event_ng, dates_file_date_string_value
):
...
@mock_s3
def test_ParquetExporter_date_with_string_chunked(
    event_ng, dates_file_date_string_value
):
    (prefix, file) = dates_file_date_string_value()
    event = event_ng(prefix, chunksize=1, schema=schema_dates)
...
def test_ParquetExporter_date_wrong_date_time_not_chunked(
    event_ng, dates_file_date_time_wrong
):
...
@mock_s3
def test_ParquetExporter_date_time_not_chunked(event_ng, dates_file_date_time):
    (prefix, file) = dates_file_date_time()
    event = event_ng(prefix, chunksize=None, schema=schema_dates)
    result = export_and_read_result(event, "schema_dates_date_time")
